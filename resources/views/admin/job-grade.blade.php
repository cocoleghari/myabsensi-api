...
@section('content')
    ...
    @if (session('error'))
        <div id="toast-error"
            class="fixed top-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 bg-white border border-rose-100 shadow-xl rounded-xl px-5 py-3.5 text-[13px] font-medium text-rose-700 max-w-lg"
            style="animation: slideDown 0.3s ease-out;">
    ...
    <div class="flex items-center justify-between mb-6 gap-3 flex-wrap">
        <div class="min-w-0">
            <h3 class="text-base font-bold text-gray-800">Job Grade</h3>
            <p class="text-[13px] text-gray-400 mt-1 font-medium">Total {{ $jobGrades->total() }} job grade terdaftar</p>
        </div>
        <div class="flex items-center gap-2 flex-wrap">
            <button type="button" onclick="openModal('import-modal')"
                class="flex items-center gap-2 px-4 py-3 rounded-xl text-[13.5px] font-semibold text-gray-600 border border-gray-200 hover:bg-gray-50 transition-colors">
            ...
            <a href="{{ route('admin.job-grade.export', request()->only('company_id')) }}"
                class="flex items-center gap-2 px-4 py-3 rounded-xl text-[13.5px] font-semibold text-gray-600 border border-gray-200 hover:bg-gray-50 transition-colors">
                <svg class="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <a href="{{ route('admin.job-grade.create') }}"
                ...
                <span class="hidden sm:inline">Tambah Job Grade</span>
    ...
    <div class="bg-white border border-gray-100 rounded-2xl p-5 mb-5 shadow-[0_2px_8px_rgba(16,24,40,0.05)]">
        <form method="GET" action="{{ route('admin.job-grade.index') }}" class="flex flex-wrap items-center gap-3.5">
            <div class="relative flex-1 min-w-[220px]">
                ...
                <input type="text" name="search" value="{{ request('search') }}"
                    placeholder="Cari nama atau kode job grade..."
            ...
            @if (request()->anyFilled(['search', 'company_id', 'status']))
                <a href="{{ route('admin.job-grade.index') }}"
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-gray-100 bg-gray-50/70">
                        <th class="text-left px-6 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Job
                            Grade</th>
                        <th class="text-left px-4 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Grade
                        </th>
                        <th class="text-left px-4 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Company
                        </th>
                ...
                <tbody>
                    @forelse ($jobGrades as $item)
                        <tr class="border-b border-gray-50 last:border-0 hover:bg-gray-50/60 transition-colors">
                            <td class="px-6 py-4">
                                <p class="text-[13.5px] font-semibold text-gray-800">{{ $item->name }}</p>
                            </td>
                            <td class="px-4 py-4">
                                <span
                                    class="text-[11.5px] px-3 py-1.5 rounded-full bg-amber-50 text-amber-700 font-semibold">
                                    Grade {{ $item->grade }}
                                </span>
                            </td>
                            <td class="px-4 py-4 text-[13.5px] text-gray-600 font-medium">
                                {{ $item->company?->name ?? '-' }}
                            </td>
                            <td class="px-4 py-4">
                                <span
                                    class="text-[11.5px] px-3 py-1.5 rounded-full bg-blue-50 text-blue-700 font-semibold">
                            ...
                            <td class="px-4 py-4">
                                @if ($item->is_active)
                                    <span
                                        class="text-[11.5px] px-3 py-1.5 rounded-full bg-emerald-50 text-emerald-700 font-semibold">Aktif</span>
                                @else
                                    <span
                                        class="text-[11.5px] px-3 py-1.5 rounded-full bg-gray-100 text-gray-500 font-semibold">Nonaktif</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.job-grade.edit', $item->id) }}"
                                    ...
                                    <form method="POST" action="{{ route('admin.job-grade.destroy', $item->id) }}"
                                        class="delete-jobgrade-form">
                                        @csrf @method('DELETE')
                                        <button type="submit"
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-12 text-center text-[13.5px] text-gray-400">Tidak ada data
                                job grade</td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <div id="import-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 backdrop-blur-sm"
        ...
        <div class="bg-white rounded-2xl shadow-xl max-w-md w-full mx-4 p-6" style="animation: popIn 0.2s ease-out;">
            <div class="flex items-center justify-between mb-1">
                <h3 class="text-[15px] font-bold text-gray-800">Import Job Grade dari Excel</h3>
            ...
            <a href="{{ route('admin.job-grade.import-template') }}"
            ...
            <form method="POST" action="{{ route('admin.job-grade.import') }}" enctype="multipart/form-data">
                @csrf
    ...
    <div id="delete-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-xl max-w-sm w-full mx-4 p-6 text-center"
            ...
            <h3 class="text-[15px] font-bold text-gray-800 mb-1.5">Hapus Job Grade?</h3>
            <p class="text-[13px] text-gray-400 mb-6">Job grade yang masih dipakai karyawan tidak dapat dihapus.</p>
            <div class="flex gap-3">
                <button type="button" id="delete-cancel-btn"
                    class="flex-1 py-2.5 rounded-xl text-[13px] font-semibold text-gray-500 border border-gray-200 hover:bg-gray-50 transition-colors">Batal</button>
    ...
    <script>
        ...
        (function() {
            ...
            document.querySelectorAll('.delete-jobgrade-form').forEach(function(form) {
