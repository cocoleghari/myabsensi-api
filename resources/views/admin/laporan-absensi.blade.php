...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6 gap-3">
        <div class="min-w-0">
            <h3 class="text-base font-bold text-gray-800">Laporan Absensi</h3>
            <p class="text-[13px] text-gray-400 mt-1 font-medium">Total {{ $absensi->total() }} record absensi</p>
        </div>
    ...
    <div class="bg-white border border-gray-100 rounded-2xl p-5 mb-5 shadow-[0_2px_8px_rgba(16,24,40,0.05)]">
        <form method="GET" action="{{ route('admin.laporan-absensi') }}" class="flex flex-wrap items-center gap-3.5">
            ...
            <select name="pusat_lokasi_id" class="filter-select text-[13.5px] border border-gray-200 rounded-xl px-4 py-3">
                <option value="">Semua Lokasi</option>
                @foreach ($pusatLokasis as $lokasi)
                    <option value="{{ $lokasi->id }}" {{ request('pusat_lokasi_id') == $lokasi->id ? 'selected' : '' }}>
                        {{ $lokasi->nama_lokasi }}
                    </option>
                @endforeach
            </select>
            ...
            <select name="tipe_absen" class="filter-select text-[13.5px] border border-gray-200 rounded-xl px-4 py-3">
                ...
                <thead>
                    <tr class="border-b border-gray-100 bg-gray-50/70">
                        <th class="text-left px-6 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">
                            Karyawan</th>
                        <th class="text-left px-4 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Tanggal
                        </th>
                        <th class="text-left px-4 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Waktu
                        </th>
                        <th class="text-left px-4 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Tipe
                        </th>
                        <th class="text-left px-4 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Lokasi
                        </th>
                ...
                <tbody>
                    @forelse ($absensi as $item)
                        <tr class="border-b border-gray-50 last:border-0 hover:bg-gray-50/60 transition-colors">
                            ...
                            <td class="px-4 py-4 text-[13.5px] text-gray-600 font-medium">{{ ucfirst($item->tipe_absen) }}
                            </td>
                            <td class="px-4 py-4 text-[13.5px] text-gray-600 font-medium">
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-4 py-12 text-center text-[13.5px] text-gray-400">Tidak ada data
