...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6 gap-3">
        <div class="min-w-0">
            <h3 class="text-sm font-bold text-gray-800">Job Level</h3>
            <p class="text-[11.5px] text-gray-400 mt-1 font-medium">Total {{ $jobLevels->total() }} job level terdaftar</p>
        </div>
    ...
    <div class="bg-white border border-gray-100 rounded-2xl p-4 mb-4 shadow-[0_2px_8px_rgba(16,24,40,0.05)]">
        <form method="GET" action="{{ route('admin.job-level.index') }}" class="flex flex-wrap items-center gap-2.5">
                ...
                <tbody>
                    @forelse ($jobLevels as $item)
                        <tr class="border-b border-gray-50 last:border-0 hover:bg-gray-50/60 transition-colors">
                            <td class="px-6 py-4 text-[13px] font-semibold text-gray-800">{{ $item->name }}</td>
    ...
    <div id="delete-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-xl max-w-sm w-full mx-4 p-5 text-center"
            ...
            <h3 class="text-[13.5px] font-bold text-gray-800 mb-1">Hapus Job Level?</h3>
            <p class="text-[12px] text-gray-400 mb-4">Job level yang masih digunakan karyawan tidak dapat dihapus.</p>
