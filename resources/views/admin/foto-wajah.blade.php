...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6 gap-3">
        <div class="min-w-0">
            <h3 class="text-base font-bold text-gray-800">Foto Dasar Kehadiran</h3>
            <p class="text-[13px] text-gray-400 mt-1 font-medium">Kelola foto referensi wajah untuk verifikasi absensi ·
    ...
    <div class="bg-white border border-gray-100 rounded-2xl p-5 mb-5 shadow-[0_2px_8px_rgba(16,24,40,0.05)]">
        <form method="GET" action="{{ route('admin.foto-wajah.index') }}" class="flex flex-wrap items-center gap-3.5">
            ...
            @if (request()->anyFilled(['search', 'department_id', 'status']))
                <a href="{{ route('admin.foto-wajah.index') }}"
                    class="text-[13px] text-gray-400 hover:text-gray-600 transition-colors font-medium">Reset Filter</a>
            @endif
        </form>
    </div>
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-gray-100 bg-gray-50/70">
                        <th class="text-left px-6 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Foto
                        </th>
                        ...
                        <tr class="border-b border-gray-50 last:border-0 hover:bg-gray-50/60 transition-colors">
                            ...
                            <td class="px-4 py-4">
                                @if ($item->wajah_terdaftar)
                                    <span
                                        class="text-[11.5px] px-3 py-1.5 rounded-full bg-emerald-50 text-emerald-700 font-semibold">Terdaftar</span>
                                @else
                                    <span
                                        class="text-[11.5px] px-3 py-1.5 rounded-full bg-gray-100 text-gray-500 font-semibold">Belum
                                        Terdaftar</span>
                                @endif
                            </td>
    ...
    <div id="reset-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-xl max-w-sm w-full mx-4 p-6 text-center"
            ...
            <h3 class="text-[15px] font-bold text-gray-800 mb-1.5">Reset Foto Wajah?</h3>
            <p id="reset-modal-name" class="text-[13px] text-gray-400 mb-6"></p>
