...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6 gap-3">
        <div class="min-w-0">
            <h3 class="text-base font-bold text-gray-800">Assign Shift Karyawan</h3>
    ...
    <div class="bg-white border border-gray-100 rounded-2xl p-5 mb-5 shadow-[0_2px_8px_rgba(16,24,40,0.05)]">
        <form method="GET" action="{{ route('admin.assign-shift.index') }}" class="flex flex-wrap items-center gap-3.5">
            {{-- Search --}}
                ...
                <thead>
                    <tr class="border-b border-gray-100 bg-gray-50/70">
                        <th class="text-left px-6 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">
                            Karyawan</th>
                        <th class="text-left px-4 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Shift /
                            Pola Shift</th>
                        <th class="text-left px-4 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Mulai
                        </th>
                        <th class="text-left px-4 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Selesai
                        </th>
                ...
                <tbody>
                    @forelse ($assignments as $item)
                        ...
                        <tr class="border-b border-gray-50 last:border-0 hover:bg-gray-50/60 transition-colors">
                            ...
                            <td class="px-4 py-4">
                                @if ($item->shift)
                                    <p class="text-[13.5px] font-semibold text-gray-800">{{ $item->shift->nama }}</p>
                                    <p class="text-[12px] text-gray-400 mt-0.5">{{ $item->shift->jam_masuk }} –
                                        {{ $item->shift->jam_pulang }}</p>
                                @elseif ($item->pattern)
                                    <p class="text-[13.5px] font-semibold text-gray-800">{{ $item->pattern->nama }}</p>
                                    <p class="text-[12px] text-gray-400 mt-0.5">
                                        <span
                                ...
                                @else
                                    <p class="text-[13.5px] text-gray-400">-</p>
                                @endif
                            </td>
                            ...
                            <td class="px-4 py-4">
                                @if ($selesai)
                                    <span
                                        class="text-[11.5px] px-3 py-1.5 rounded-full bg-gray-100 text-gray-500 font-semibold">Selesai</span>
                                @else
                                    <span
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-12 text-center text-[13.5px] text-gray-400">
    ...
    <div id="delete-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-xl max-w-sm w-full mx-4 p-6 text-center"
            ...
            <h3 class="text-[15px] font-bold text-gray-800 mb-1.5">Hapus Assignment?</h3>
            <p class="text-[13px] text-gray-400 mb-6">Data assignment shift ini akan dihapus permanen.</p>
