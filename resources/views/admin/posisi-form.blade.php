@extends('layouts.admin')
@section('title', isset($position) ? 'Edit Posisi' : 'Tambah Posisi')

@section('content')
    <div class="flex items-center gap-2 mb-6 text-xs text-gray-400">
        <a href="{{ route('admin.posisi.index') }}" class="hover:text-gray-600 transition">Posisi / Jabatan</a>
        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
        </svg>
        <span class="text-gray-600">{{ isset($position) ? 'Edit Posisi' : 'Tambah Posisi' }}</span>
    </div>

    <div class="bg-white border border-gray-100 rounded-2xl p-6 shadow-[0_2px_8px_rgba(16,24,40,0.05)]">
        @if ($errors->any())
            <div class="mb-5 bg-red-50 border border-red-100 rounded-xl px-4 py-3">
                <ul class="text-xs text-red-600 list-disc list-inside space-y-0.5">
                    @foreach ($errors->all() as $error)
                        <li>{{ $error }}</li>
                    @endforeach
                </ul>
            </div>
        @endif

        <form method="POST"
            action="{{ isset($position) ? route('admin.posisi.update', $position->id) : route('admin.posisi.store') }}">
            @csrf
            @if (isset($position))
                @method('PUT')
            @endif

            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                <div>
                    <label class="form-label">Nama Posisi <span class="text-red-400">*</span></label>
                    <input type="text" name="name" value="{{ old('name', $position->name ?? '') }}" class="form-input"
                        required>
                </div>

                <div>
                    <label class="form-label">Company <span class="text-red-400">*</span></label>
                    <select name="company_id" class="form-input" required>
                        <option value="">-- Pilih Company --</option>
                        @foreach ($companies as $c)
                            <option value="{{ $c->id }}"
                                {{ old('company_id', $position->company_id ?? '') == $c->id ? 'selected' : '' }}>
                                {{ $c->name }}
                            </option>
                        @endforeach
                    </select>
                </div>

                <div>
                    <label class="form-label">Kode <span class="text-gray-400 font-normal">(opsional)</span></label>
                    <input type="text" name="code" value="{{ old('code', $position->code ?? '') }}"
                        class="form-input">
                </div>

                <div>
                    <label class="form-label">Urutan <span class="text-gray-400 font-normal">(opsional)</span></label>
                    <input type="number" name="order" value="{{ old('order', $position->order ?? 0) }}"
                        class="form-input">
                </div>

                <div class="md:col-span-2">
                    <label class="form-label">Deskripsi <span class="text-gray-400 font-normal">(opsional)</span></label>
                    <textarea name="description" rows="3" class="form-input">{{ old('description', $position->description ?? '') }}</textarea>
                </div>

                <div class="md:col-span-2 flex items-center gap-2">
                    <input type="checkbox" name="is_active" value="1" id="is_active"
                        {{ old('is_active', $position->is_active ?? true) ? 'checked' : '' }}
                        class="w-4 h-4 rounded border-gray-300 text-orange-500 focus:ring-orange-400">
                    <label for="is_active" class="text-[12.5px] text-gray-600">Posisi aktif</label>
                </div>
            </div>

            <div class="flex items-center gap-3 mt-7 pt-5 border-t border-gray-100">
                <button type="submit"
                    class="flex items-center gap-2 text-xs font-medium px-5 py-2.5 rounded-lg text-white hover:opacity-90 transition"
                    style="background:#f97316">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                    </svg>
                    {{ isset($position) ? 'Update Posisi' : 'Simpan Posisi' }}
                </button>
                <a href="{{ route('admin.posisi.index') }}" class="text-xs text-gray-400 hover:text-gray-600">Batal</a>
            </div>
        </form>
    </div>

    <style>
        .form-label {
            display: block;
            font-size: 11px;
            font-weight: 500;
            color: #4B5563;
            margin-bottom: 6px;
        }

        .form-input {
            width: 100%;
            font-size: 12px;
            border: 1px solid #E5E7EB;
            border-radius: 10px;
            padding: 10px 13px;
            outline: none;
            transition: border-color .15s;
            background: #fff;
        }

        .form-input:focus {
            border-color: #f97316;
            box-shadow: 0 0 0 3px rgba(249, 115, 22, .08);
        }
    </style>
@endsection
