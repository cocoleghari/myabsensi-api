...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6 gap-3">
        <div class="min-w-0">
            <h3 class="text-sm font-bold text-gray-800">Pola Shift Mingguan</h3>
            <p class="text-[11.5px] text-gray-400 mt-1 font-medium">Total {{ $patterns->total() }} pola terdaftar</p>
        </div>
    ...
    <div class="bg-white border border-gray-100 rounded-2xl p-4 mb-4 shadow-[0_2px_8px_rgba(16,24,40,0.05)]">
        <form method="GET" action="{{ route('admin.pola-shift.index') }}" class="flex flex-wrap items-center gap-2.5">
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-gray-100 bg-gray-50/70">
                        <th class="text-left px-4 py-3 text-[10px] font-bold text-gray-400 uppercase tracking-wider">Pola
                        </th>
                        <th class="text-left px-4 py-3 text-[10px] font-bold text-gray-400 uppercase tracking-wider">Company
                        </th>
                        <th class="text-left px-4 py-3 text-[10px] font-bold text-gray-400 uppercase tracking-wider">Hari
                            Terisi</th>
                ...
                <tbody>
                    @forelse ($patterns as $item)
                        <tr class="border-b border-gray-50 last:border-0 hover:bg-gray-50/60 transition-colors">
                            <td class="px-4 py-3">
                                <p class="text-[13px] font-semibold text-gray-800">{{ $item->nama }}</p>
                                <p class="text-[11px] text-gray-400 mt-0.5">{{ $item->kode }}</p>
                            </td>
    ...
    <div id="delete-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-xl max-w-sm w-full mx-4 p-5 text-center"
            ...
            <h3 class="text-[13.5px] font-bold text-gray-800 mb-1">Hapus Pola Shift?</h3>
            <p class="text-[12px] text-gray-400 mb-4">Pola yang masih digunakan karyawan tidak dapat dihapus.</p>
