...
@section('title', 'Pola Shift Mingguan')
...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6 gap-3">
        <div class="min-w-0">
            <h3 class="text-base font-bold text-gray-800">Pola Shift Mingguan</h3>
            <p class="text-[13px] text-gray-400 mt-1 font-medium">Total {{ $patterns->total() }} pola terdaftar</p>
        </div>
        <a href="{{ route('admin.pola-shift.create') }}"
            ...
            <span class="hidden sm:inline">Tambah Pola</span>
    ...
    <div class="bg-white border border-gray-100 rounded-2xl p-5 mb-5 shadow-[0_2px_8px_rgba(16,24,40,0.05)]">
        <form method="GET" action="{{ route('admin.pola-shift.index') }}" class="flex flex-wrap items-center gap-3.5">
            <div class="relative flex-1 min-w-[220px]">
                ...
                <input type="text" name="search" value="{{ request('search') }}"
                    placeholder="Cari nama atau kode pola..."
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-gray-100 bg-gray-50/70">
                        <th class="text-left px-6 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Pola
                        </th>
                        <th class="text-left px-4 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Company
                        </th>
                        <th class="text-left px-4 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Hari
                            Terisi</th>
                ...
                <tbody>
                    @forelse ($patterns as $item)
                        <tr class="border-b border-gray-50 last:border-0 hover:bg-gray-50/60 transition-colors">
                            <td class="px-6 py-4">
                                <p class="text-[13.5px] font-semibold text-gray-800">{{ $item->nama }}</p>
                                <p class="text-[12px] text-gray-400 mt-0.5">{{ $item->kode }}</p>
                            </td>
                            <td class="px-4 py-4 text-[13.5px] text-gray-600 font-medium">
                                {{ $item->company?->name ?? '-' }}</td>
                            <td class="px-4 py-4 text-[13px] text-gray-600">{{ $item->days_count }} / 7 hari</td>
                            <td class="px-4 py-4">
                                <span class="text-[11.5px] px-3 py-1.5 rounded-full bg-blue-50 text-blue-700 font-semibold">
                                    {{ $item->employee_shifts_count }} orang
                                </span>
                            </td>
                            <td class="px-4 py-4">
                                @if ($item->is_active)
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.pola-shift.edit', $item->id) }}"
                                    ...
                                    <form method="POST" action="{{ route('admin.pola-shift.destroy', $item->id) }}"
                                        class="delete-pattern-form">
                                        @csrf @method('DELETE')
                                        <button type="submit"
    ...
    <div id="delete-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-xl max-w-sm w-full mx-4 p-6 text-center"
            ...
            <h3 class="text-[15px] font-bold text-gray-800 mb-1.5">Hapus Pola Shift?</h3>
            <p class="text-[13px] text-gray-400 mb-6">Pola yang masih digunakan karyawan tidak dapat dihapus.</p>
    ...
    <script>
        ...
        (function() {
            ...
            document.querySelectorAll('.delete-pattern-form').forEach(function(form) {
