...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6 gap-3">
        <div class="min-w-0">
            <h3 class="text-base font-bold text-gray-800">Status Karyawan</h3>
            <p class="text-[13px] text-gray-400 mt-1 font-medium">Total {{ $statuses->total() }} status terdaftar</p>
        </div>
        <a href="{{ route('admin.status-karyawan.create') }}"
            ...
            <span class="hidden sm:inline">Tambah Status</span>
    ...
    <div class="bg-white border border-gray-100 rounded-2xl p-5 mb-5 shadow-[0_2px_8px_rgba(16,24,40,0.05)]">
        <form method="GET" action="{{ route('admin.status-karyawan.index') }}"
            ...
            <div class="relative flex-1 min-w-[220px]">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari label atau kode..."
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-gray-100 bg-gray-50/70">
                        <th class="text-left px-6 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Label
                        </th>
                        <th class="text-left px-4 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Kode
                        </th>
                        <th class="text-left px-4 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">
                            Karyawan</th>
                        <th class="text-left px-4 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Tampil
                            di Mobile</th>
                        ...
                        <tr class="border-b border-gray-50 last:border-0 hover:bg-gray-50/60 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2.5">
                                    ...
                                    @endif
                                    <p class="text-[13.5px] font-semibold text-gray-800">{{ $item->label }}</p>
                                </div>
                            </td>
                            <td class="px-4 py-4 text-[13px] text-gray-500 font-medium">{{ $item->code }}</td>
                            <td class="px-4 py-4">
                            ...
                            <td class="px-4 py-4">
                                @if ($item->is_visible)
                                    <span
                                        class="text-[11.5px] px-3 py-1.5 rounded-full bg-blue-50 text-blue-700 font-semibold">Ya</span>
                                @else
                                    <span
                                        class="text-[11.5px] px-3 py-1.5 rounded-full bg-gray-100 text-gray-500 font-semibold">Tidak</span>
                                @endif
                            </td>
                            <td class="px-4 py-4">
                                @if ($item->is_active)
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.status-karyawan.edit', $item->id) }}"
                                    ...
                                    <form method="POST" action="{{ route('admin.status-karyawan.destroy', $item->id) }}"
    ...
    <div id="delete-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-xl max-w-sm w-full mx-4 p-6 text-center"
            ...
            <h3 class="text-[15px] font-bold text-gray-800 mb-1.5">Hapus Status Karyawan?</h3>
            <p class="text-[13px] text-gray-400 mb-6">Status yang masih digunakan karyawan tidak dapat dihapus.</p>
    ...
    <script>
        ...
        (function() {
            ...
            document.querySelectorAll('.delete-status-form').forEach(function(form) {
